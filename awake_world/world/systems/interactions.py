...
class InteractionSystem:
    """Contextual target scoring with hysteresis and forgiving range."""
    ...
    @staticmethod
    def _score(item: InteractionLike, x: float, y: float, facing: tuple[float, float] | None) -> float:
        dx = float(item.x) - x
        dy = float(item.y) - y
        ...
        return distance_score + facing_score + priority
    ...
    def nearest(
        ...
        for item in items:
            score = self._score(item, x, y, facing)
            if score > best_score:
        ...
        if current is not None and current in items:
            ...
            current_score = self._score(current, x, y, facing)
            if dist <= current.radius * self.linger_multiplier:
